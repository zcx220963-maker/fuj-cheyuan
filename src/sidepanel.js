(function () {
  ...
  async function startOrContinueBatch() {
    ...
    await runBusy('开始批量录入...', async () => {
      ...
      if (state.rows[state.currentIndex]) {
        ...
        rows[state.currentIndex] = { ...rows[state.currentIndex], brandConfirmed: false };
        state = await VehicleStore.save({ rows });
        logLine(`[品牌确认] 用户主动开始批量 → 重置当前行 brandConfirmed=false：本次车源新增页品牌需手动选择，创建车系/车型及重填将自动沿用。`, 'info');
      }
  ...
  async function fillCurrentWithNavigation() {
    ...
    if (combinedGoCreate) {
      ...
      logLine(
        ...
        `按你要求：**其他字段一概不填，立刻跳去创建**（不 waitingManualSubmit 卡死，不 Stage2/3 填垃圾字段）。` +
      ...
      try {
        // ---- 1) 先创车系 ----
        if (sMiss && row.brandName && row.seriesName) {
          ...
          logLine(`[砍屎山] 直达创建车系 → 品牌=${row.brandName} 车系=${row.seriesName} 车辆类型=${row.vehicleType || '(留空，抽屉里选)'} | skipReturn=${skipReturn1 ? '✅是（后面还有车型要创建）' : '否（直接回新增页）'} | switchToModel=${switchToModel1 ? '✅是（建完自动切到车型管理列表/Model/Model页面）' : '否'}`, 'info');
          const r1 = await sendToActiveTab({
            type: 'VA_NAVIGATE_AND_CREATE',
            payload: { kind: 'series', brandName: row.brandName, brandConfirmed: !!row.brandConfirmed, seriesName: row.seriesName, vehicleType: row.vehicleType || '', skipReturnToAddPage: skipReturn1, switchToModelAfterSeriesCreated: switchToModel1 },
            timeout: 180000,
          });
          ...
          needBack = true;
        } else if (sMiss && (!row.brandName || !row.seriesName)) {
          logLine(`[砍屎山] ⚠️ 车系未命中但 row.brandName/row.seriesName 为空（测试表没填），无法自动创建，停 waitingManualSubmit 请人工处理。`, 'warn');
        }
        // ---- 2) 再创车型 ----
        if (mMiss && row.brandName && row.seriesName && row.modelName) {
          // ★ 车型创建也【必须传 skipReturnToAddPage:true】
          logLine(`[砍屎山] 直达创建车型 → 品牌=${row.brandName} 车系=${row.seriesName} 车型=${row.modelName} 车辆类型=${row.vehicleType || '(留空，抽屉里选)'} | skipReturn=✅是（由sidepanel needBack块统一回车源+重填，防content-admin和sidepanel两次跳打架）`, 'info');
          const r2 = await sendToActiveTab({
            type: 'VA_NAVIGATE_AND_CREATE',
            payload: { kind: 'model', brandName: row.brandName, brandConfirmed: !!row.brandConfirmed, seriesName: row.seriesName, modelName: row.modelName, vehicleType: row.vehicleType || '', skipReturnToAddPage: true },
            timeout: 180000,
          });
  ...
  async function fillCurrentRowCore(opts) {
    ...
    if (selectedBrandFromReport) {
      ...
      if (sb !== row.brandName) {
        ...
        state = await VehicleStore.save({ rows });
        logLine(`[品牌复用] 用户已确认品牌「${sb}」，写回当前行 → 创建车系/车型及重填均自动沿用，不再人工操作`, 'ok');
        row.brandName = sb;
        row.brandConfirmed = true;
      } else if (sb === row.brandName) {
        // 品牌名一致也标记已确认，确保创建车系/车型时走自动填
        if (!row.brandConfirmed) {
          const rows = [...state.rows];
          rows[state.currentIndex] = { ...rows[state.currentIndex], brandConfirmed: true };
          state = await VehicleStore.save({ rows });
          row.brandConfirmed = true;
        }
      }
  ...
  async function tryCreateMissingSeriesOrModel(row, report) {
    ...
    if (seriesMiss && row.brandName && row.seriesName) {
      // ★ 决定本次车系创建完是否马上回新增页：
      ...
      const skipReturnBecauseModelNext = !!(modelMiss && row.modelName && row.brandName && row.seriesName);
      // 同步：车系创建完若后面还有车型，要切到车型管理列表页（switchToModelAfterSeriesCreated=true）
      const switchToModelNext = !!skipReturnBecauseModelNext;
      logLine(`[创建车系] 开始 → 品牌="${row.brandName}"，车系="${row.seriesName}"，车辆类型="${row.vehicleType || '-'}") | skipReturn=${skipReturnBecauseModelNext ? '✅ 是（后面还有车型创建）' : '否（后面没车型，本步创建完直接回新增页）'} | switchToModel=${switchToModelNext ? '✅ 是（车系创建完切到车型管理列表，不再停留车系列表）' : '否'}`, 'warn');
      const t0 = Date.now();
      try {
        const resp = await sendToActiveTab({
          type: 'VA_NAVIGATE_AND_CREATE',
          payload: {
            ...
            brandConfirmed: !!row.brandConfirmed,   // ★ 品牌已在车源新增页确认过 → 创建车系自动填品牌，不再人工操作
    ...
    if (modelMiss && row.brandName && row.seriesName && row.modelName) {
      if (!seriesMiss || seriesCreated) {
        logLine(`[创建车型] 开始 → 品牌="${row.brandName}"，车系="${row.seriesName}"，车型="${row.modelName}"）`, 'warn');
        const t1 = Date.now();
        try {
          const resp = await sendToActiveTab({
            type: 'VA_NAVIGATE_AND_CREATE',
            payload: {
              kind: 'model',
              brandName: row.brandName,
              brandConfirmed: !!row.brandConfirmed,   // ★ 品牌已在车源新增页确认过 → 创建车型自动填品牌，不再人工操作
              seriesName: row.seriesName,
