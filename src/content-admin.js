(function () {
  ...
  async function createSeries(payload) {
    ...
    if (brandName) {
      ...
      if (!rBrand.ok) {
        if (brandConfirmed) {
          return { ok: false, message: `品牌一致性锁：车源新增页已确认品牌「${brandName}」，但创建车系抽屉无法自动选中该品牌，已停止创建，避免创建到其他品牌下面。`, steps };
        }
    ...
    const notice = addNoticeInsideModal(modal, `请手动选择品牌并提交`);
  ...
  async function createModel(payload) {
    ...
    if (brandName) {
      ...
      if (!rBrand.ok) {
        if (brandConfirmed) {
          return { ok: false, message: `品牌一致性锁：车源新增页已确认品牌「${brandName}」，但创建车型抽屉无法自动选中该品牌，已停止创建，避免创建到其他品牌下面。`, steps };
        }
        // 自动填失败才回退手动（下拉里没有该品牌，必须让用户从系统里选一个真实存在的）
        console.log(`[创建车型] 品牌「${brandName}」自动填失败 → 回退 ensureBrandSelected 等待手动选择`);
        const rManual = await ensureBrandSelected(modal, {
          suggestText: '创建车型前请先手动选择品牌，完成后插件会继续填写车型信息。',
        });
        if (rManual.ok) {
          rBrand = { ...rManual, auto: true };
          steps.push(`  ↳ 回退成功：你已选择「${rManual.value}」`);
    ...
    const notice = addNoticeInsideModal(modal, `请手动选择品牌并确认车系`);
    steps.push('[人工确认] 车系名称/车型名称/类型 已填完，等待手动选择品牌并提交');
    console.log('[VA 车型创建] 字段已填完，等待人工提交确认：', { actualBrandName, seriesName, modelName, vehicleType });
  ...
  function addNoticeInsideModal(scope, htmlOrText) {
    ...
    try {
      ...
      Object.assign(bar.style, {
        ...
        boxSizing: 'border-box', width: '100%', maxWidth: 'none',
        padding: '18px 22px', margin: '0 0 18px',
        background: 'linear-gradient(90deg,#fff7ed,#fef3c7)',
        border: '2px solid #f59e0b', color: '#92400e',
        borderRadius: '10px', fontWeight: '700', fontSize: '18px', lineHeight: '1.6',
        letterSpacing: '0.2px', boxShadow: '0 10px 22px rgba(0,0,0,0.08)',
      });
  ...
  async function waitForManualDropdown(scope, opts) {
    ...
    Object.assign(bar.style, {
      ...
      boxSizing: 'border-box', width: '100%', maxWidth: 'none',
      padding: '18px 24px', margin: '18px 0 22px',
      background: 'linear-gradient(90deg,#eff6ff,#dbeafe)',
      border: '2px solid #3b82f6', color: '#1e3a8a',
      borderRadius: '10px', fontWeight: '700', fontSize: '20px', lineHeight: '1.55',
      letterSpacing: '0.2px', boxShadow: '0 10px 24px rgba(30,64,175,0.16)',
    });
